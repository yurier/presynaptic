...
def vesicle_release_with_decay(D0, R0, tau_adap, delta, tau_D, tau_R, tau_refR, h, s, decay_rate, jump_size, T, dt, release_rate, max_attempts, quiet_duration):
    ...
    max_iterations = 10#int((T + quiet_duration)/dt)  # Just as a safety net
    iteration = 0
    ...
    while t < T + quiet_duration:
        ...
        print(t < next_release_time, t < T + quiet_duration)
        ...
        if release_attempts == max_attempts:
            ...
            #print(release_attempts,max_attempts)
...
release_rate = 30
max_attempts=300
T = 1000*(max_attempts/release_rate) #time the spike train is going to finish
quiet_duration=1000*100
...
# Calcium dynamics with jumps and decay
plt.subplot(4, 1, 2)
...
# Steps per second
plt.subplot(4, 1, 4)
steps_per_second = [1/dt] * len(times)
plt.plot(times, steps_per_second, label=f"Steps per second: {1/dt}")
plt.xlabel('Time')
plt.ylabel('Steps/sec')
plt.title('Number of Steps per Second')
plt.grid(True)
plt.legend()
plt.tight_layout()
